...
@dataset_constructor.register("GEM/viggo", "gem/viggo")
def viggo_preprocessing_function(inp: Dict):
    try:
        full_prompt = (f"Given a target sentence construct the underlying meaning representation of the input sentence as a single function with attributes and attribute values. "
                       f"This function should describe the target string accurately and the function must be one of the following ['inform', 'request', 'give_opinion', 'confirm', 'verify_attribute', 'suggest', 'request_explanation', 'recommend', 'request_attribute']. "
                       f"The attributes must be one of the following: ['name', 'exp_release_date', 'release_year', 'developer', 'esrb', 'rating', 'genres', 'player_perspective', 'has_multiplayer', 'platforms', 'available_on_steam', 'has_linux_release', 'has_mac_release', 'specifier']"
                       f"\n\n### Target sentence:\n{inp['target']}\n\n### Meaning representation:\n")
    except Exception as e:
        raise ValueError(
            f"Unable to extract prompt/response from inp={inp}"
        ) from e
    return {'prompt': full_prompt, 'response': inp['meaning_representation']}
