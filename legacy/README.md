# ICML 2024 submission code (frozen)

This directory contains the original RoSA implementation used for the paper
["RoSA: Accurate Parameter-Efficient Fine-Tuning via Robust Adaptation"](https://arxiv.org/abs/2401.04679).

It is a fork of [MosaicML llm-foundry](https://github.com/mosaicml/llm-foundry) with Composer-based training scripts.

- **Unmodified history**: see the `icml2024` git branch.
- **New HuggingFace + TRL code**: `src/rosa/` and `examples/` on the `refactor` branch.
...
2. Install a version of [pytorch](https://pytorch.org/) (>=2.1.2) compatible with your CUDA.

3. Install this legacy tree:

```bash
pip install -e legacy/
```

(from repo root, using `legacy/setup.py`)

4. Install [spops](https://github.com/IST-DASLab/spops) and [peft-rosa](https://github.com/IST-DASLab/peft-rosa) as described in the original README below.

## Quick Start (legacy)

```bash
conda activate rosa
cd legacy/scripts/train/
```

See the rest of this file for dataset-specific training and evaluation commands.
